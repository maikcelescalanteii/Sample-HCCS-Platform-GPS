...
import com.hccs.samples.sample.gps.node.dashboard.details._DashboardDetailsGlass;
...
@Glass
public class _DashboardListGlass extends BorderPane implements DashboardListGlass {

	@FXML
	ListView<DashboardAdapter> lstStudents;

	List<StudentBean> mLstStudents;
	
	private Consumer<DashboardAdapter> onChangeData;
	
	@Override
	public void init() {
		System.out.println("Loading _ListGlass, init method.");
		FXMLLoader loader = new FXMLLoader(_DashboardListGlass.class.getResource("/com/hccs/sample/resources/fxml/StudentList.fxml"));
		...
		lstStudents.getSelectionModel().selectedItemProperty()
			.addListener((ChangeListener<DashboardAdapter>) (obs, oldVal, newVal) -> {
			if (newVal == null) {
	...
	public GlassGrabber<Node> getGlassGrabber() {
		return new GlassGrabber<Node>() {
			...
			public Node getComponent() {
				return _DashboardListGlass.this;
			}
		};
	}
